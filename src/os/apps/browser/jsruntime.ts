...
export function createPageJS(
  ...
  var _childBootstrap = [
    ...
    '  this.classList = _cls;',
    '  var _styleBase = {};',
    '  _styleBase.setProperty = function(k,v){ _styleBase[k.trim()]=v; _domDirty=true; };',
    '  _styleBase.getPropertyValue = function(k){ return _styleBase[k.trim()]||""; };',
    '  _styleBase.getPropertyPriority = function(){ return ""; };',
    '  _styleBase.removeProperty = function(k){ var v=_styleBase[k.trim()]||""; delete _styleBase[k.trim()]; _domDirty=true; return v; };',
    '  Object.defineProperty(_styleBase,"cssText",{get:function(){var r=[];for(var k in _styleBase){if(typeof _styleBase[k]==="string")r.push(k+":"+_styleBase[k]);}return r.join(";");},set:function(t){for(var k in _styleBase){if(typeof _styleBase[k]==="string")delete _styleBase[k];}String(t).split(";").forEach(function(p){var i=p.indexOf(":");if(i>0){var k2=p.slice(0,i).trim(),v=p.slice(i+1).trim();if(k2)_styleBase[k2]=v;}});_domDirty=true;},enumerable:false,configurable:true});',
    '  _styleBase.item = function(i){ var ks=Object.keys(_styleBase).filter(function(k){return typeof _styleBase[k]==="string";}); return ks[i]||""; };',
    '  var _style = typeof Proxy!=="undefined" ? new Proxy(_styleBase,{set:function(t,k,v){t[k]=v;if(typeof v!=="function")_domDirty=true;return true;}}) : _styleBase;',
    '  this.style = _style;',
    '  this.dataset = {};',
    '  this._attrs = {};',
    '  this.textContent = "";',
    ...
    '_StubElement.prototype.appendChild = function(c){ if(c.nodeType===11){var ch=c.childNodes.slice();c.childNodes=[];for(var i=0;i<ch.length;i++)this.appendChild(ch[i]);return c;} if(c.parentNode&&c.parentNode!==this) c.parentNode.removeChild(c); this.childNodes.push(c); c.parentNode=this; c.parentElement=this; _linkAll(this); _dispatchMO("childList",this,[c],[]); return c; };',
    ...
    '_StubElement.prototype.insertBefore = function(n,r){ if(!r) return this.appendChild(n); if(n.nodeType===11){var ch=n.childNodes.slice();n.childNodes=[];for(var j=0;j<ch.length;j++)this.insertBefore(ch[j],r);return n;} if(n.parentNode&&n.parentNode!==this) n.parentNode.removeChild(n); var i=this.childNodes.indexOf(r); if(i>=0) this.childNodes.splice(i,0,n); else this.childNodes.push(n); n.parentNode=this; n.parentElement=this; _linkAll(this); _dispatchMO("childList",this,[n],[]); return n; };',
    '_StubElement.prototype.replaceChild = function(n,o){ if(n.nodeType===11){var ch=n.childNodes.slice();n.childNodes=[];if(ch.length===0){this.removeChild(o);return o;}var i=this.childNodes.indexOf(o);if(i<0)return o;this.childNodes[i]=ch[0];ch[0].parentNode=this;ch[0].parentElement=this;o.parentNode=null;o.parentElement=null;for(var j=1;j<ch.length;j++){this.childNodes.splice(i+j,0,ch[j]);ch[j].parentNode=this;ch[j].parentElement=this;}_linkAll(this);_dispatchMO("childList",this,ch,[o]);return o;} if(n.parentNode&&n.parentNode!==this) n.parentNode.removeChild(n); var i=this.childNodes.indexOf(o); if(i>=0){ this.childNodes[i]=n; n.parentNode=this; n.parentElement=this; o.parentNode=null; o.parentElement=null; _linkAll(this); _dispatchMO("childList",this,[n],[o]); } return o; };',
    ...
    '  createDocumentFragment: function(){ var f=new _StubElement("FRAGMENT"); f.nodeType=11; f.nodeName="#document-fragment"; return f; },',
    '  createTextNode: function(t){ var n={nodeType:3,textContent:String(t),nodeValue:String(t),parentNode:null,parentElement:null,nextSibling:null,previousSibling:null}; Object.defineProperty(n,"data",{get:function(){return n.nodeValue;},set:function(v){n.nodeValue=String(v);n.textContent=String(v);_domDirty=true;_dispatchMO("characterData",n,[],[]);},configurable:true,enumerable:true}); n.data=String(t); return n; },',
    ...
    'Event.prototype.preventDefault=function(){if(this._inPassive)return;if(this.cancelable)this.defaultPrevented=true;};',
    ...
    // ── Misc Web APIs ────────────────────────────────────────────────────
    'var _moRegistry = [];',
    'var _fireMOAsync = function(mo,rec){ setTimeout(function(){ try{ mo._cb([rec],mo); }catch(e){} },0); };',
    'var _dispatchMO = function(type,target,added,removed,attrName){',
    '  var rec={type:type,target:target,addedNodes:added||[],removedNodes:removed||[],attributeName:attrName||null,oldValue:null,nextSibling:null,previousSibling:null};',
    '  for(var _mi=0;_mi<_moRegistry.length;_mi++){var _mo=_moRegistry[_mi];',
    '    for(var _mj=0;_mj<_mo._nodes.length;_mj++){var _mn=_mo._nodes[_mj];',
    '      if(type==="characterData"&&_mn.opts.characterData&&_mn.node===target){_fireMOAsync(_mo,rec);}',
    '      else if(type==="attributes"&&_mn.opts.attributes&&(_mn.node===target||_mn.opts.subtree)){_fireMOAsync(_mo,rec);}',
    '      else if(type==="childList"&&_mn.opts.childList&&(_mn.node===target||(_mn.opts.subtree&&(function(n){while(n=n.parentNode){if(n===_mn.node)return true;}return false;})(target)))){_fireMOAsync(_mo,rec);}',
    '    }',
    '  }',
    '};',
    'var MutationObserver = function(cb){ this._cb=cb; this._nodes=[]; _moRegistry.push(this); };',
    'MutationObserver.prototype.observe = function(node,opts) { this._nodes.push({node:node,opts:opts||{}}); };',
    'MutationObserver.prototype.disconnect = function() { this._nodes=[]; var i=_moRegistry.indexOf(this); if(i>=0)_moRegistry.splice(i,1); };',
    ...
    'EventTarget.prototype.addEventListener = function(t,fn,opts) { if(!this._listeners) this._listeners={}; var _once=opts&&typeof opts==="object"&&opts.once; var _passive; if(opts&&typeof opts==="object"&&opts.passive!==undefined){_passive=!!opts.passive;}else{_passive=(t==="touchstart"||t==="touchmove"||t==="wheel"||t==="mousewheel");} var _fn=_once?function(e){fn.call(this,e);this.removeEventListener(t,_fn);}.bind(this):fn; if(!this._listeners[t]) this._listeners[t]=[]; this._listeners[t].push({fn:_fn,orig:fn,passive:_passive}); };',
    ...
    'EventTarget.prototype.dispatchEvent = function(ev) { if(!this._listeners) this._listeners={}; var _type=ev.type; ev.target=ev.target||this; ev.currentTarget=this; var _oh=this["on"+_type]; if(typeof _oh==="function")try{_oh.call(this,ev);}catch(_){} var _ls=this._listeners[_type]; if(_ls){var _lsc=_ls.slice();for(var _li=0;_li<_lsc.length;_li++){if(ev._stopImm)break;ev._inPassive=!!_lsc[_li].passive;try{_lsc[_li].fn.call(this,ev);}catch(_){}ev._inPassive=false;}} if(ev.bubbles&&!ev.cancelBubble){var _ep=this.parentNode;while(_ep&&!ev.cancelBubble){ev.currentTarget=_ep;var _poh=_ep["on"+_type];if(typeof _poh==="function")try{_poh.call(_ep,ev);}catch(_){}if(_ep._listeners&&_ep._listeners[_type]){var _pls=_ep._listeners[_type].slice();for(var _pli=0;_pli<_pls.length;_pli++){if(ev._stopImm||ev.cancelBubble)break;ev._inPassive=!!_pls[_pli].passive;try{_pls[_pli].fn.call(_ep,ev);}catch(_){}ev._inPassive=false;}} _ep=_ep.parentNode;}} return !ev.defaultPrevented; };',
