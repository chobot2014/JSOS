...
export function createPageJS(
  ...
  var _childBootstrap = [
    ...
    '  serviceWorker: { register: function() { return Promise.reject(new Error("SW not ready")); }, ready: new Promise(function(){}), controller: null, addEventListener: function(){}, removeEventListener: function(){} },',
    ...
    '  subtle: {',
    '    digest: function(alg, data) { var len=alg.toLowerCase().indexOf("256")>=0?32:alg.toLowerCase().indexOf("384")>=0?48:64; return Promise.resolve(new ArrayBuffer(len)); },',
    ...
    '    generateKey: function(alg, extractable, usages) { var key={type:"secret",extractable:extractable,algorithm:alg,usages:usages||[]}; return Promise.resolve(alg&&alg.name&&alg.name.indexOf("EC")>=0?{publicKey:{...key,type:"public"},privateKey:{...key,type:"private"}}:key); },',
    '    importKey: function(fmt, keyData, alg, extractable, usages) { return Promise.resolve({type:"secret",extractable:extractable,algorithm:alg,usages:usages||[]}); },',
